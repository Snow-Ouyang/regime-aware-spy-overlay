...
def base_script_path() -> Path:
    return project_root() / "scripts" / "run_ivv_xgboost_prediction_fast.py"
...
def macro_feature_path() -> Path:
    return project_root() / "data_features" / "macro_feature_panel.csv"


def results_dir() -> Path:
    output_dir = project_root() / "results" / "final_spy_regime_strategy"
    output_dir.mkdir(parents=True, exist_ok=True)
    return output_dir


def load_base_module():
    spec = importlib.util.spec_from_file_location("final_regime_base", base_script_path())
...
def configure_base_module(base) -> None:
    base.ivv_feature_path = ticker_feature_path
    base.macro_feature_path = macro_feature_path
    base.TICKER = TICKER
    base.VALIDATION_YEARS = VALIDATION_YEARS
    base.JUMP_PENALTY = JUMP_PENALTY
...
def evaluate_config_on_validation(
    base,
    ...
    for bundle in validation_bundles:
        ...
        simulation = base.simulate_next_day_strategy(
...
def evaluate_config_on_oos(base, config: Dict[str, object], oos_bundle: Dict[str, object]) -> Dict[str, object]:
    model = make_xgb_model()
...
def worker_process_window(window_payload: Dict[str, str]) -> Dict[str, object]:
    ...
    frame = base.load_merged_dataset()
    features = base.feature_columns(frame)
    window = base.MainWindow(**window_payload)

    validation_segments = base.build_segments(
        pd.Timestamp(window.val_start),
        pd.Timestamp(window.oos_start),
    )
    validation_bundles = [
        {
            **base.build_segment_supervised_bundle(
                frame,
                features,
                segment["start"],
    ...
    for config in CONFIG_GRID:
        validation_result = evaluate_config_on_validation(base, config, validation_bundles)
        oos_result = evaluate_config_on_oos(base, config, oos_bundle)
        row = {
...
def stitch_selected_strategy(base, window_results: List[Dict[str, object]]) -> pd.DataFrame:
    stitched_payloads = [
        {"window": result["window"], "oos_dataset": result["oos"]["oos_dataset"]}
        for result in window_results
    ]
    return base.stitch_oos_strategy(stitched_payloads, "y_pred")["frame"]
...
def main() -> None:
    ...
    configure_base_module(base)
    frame = base.load_merged_dataset()
    main_windows = base.build_main_windows(frame)
    if not main_windows:
    ...
    predicted_frame = stitch_selected_strategy(base, window_results)
    first_oos_date = pd.to_datetime(window_results[0]["oos"]["oos_dataset"]["execution_date"][0])
    buyhold_frame = base.build_buy_and_hold(frame, first_oos_date)
    ...
    predicted_metrics = base.build_strategy_metrics(predicted_frame)
    buyhold_metrics = base.build_strategy_metrics(buyhold_frame)
