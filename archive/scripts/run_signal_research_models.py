...
from final_multi_asset_project_common import (
    FIXED_XGB_PARAMS,
    RESEARCH_ASSETS,
    SMOOTHING_HALFLIFE_GRID,
    THRESHOLD_GRID,
    TARGET_OOS_START,
    VALIDATION_YEARS,
    ensure_research_inputs,
    load_research_experiment_frame,
    research_feature_columns,
    signal_asset_results_dir,
    signal_panels_dir,
    signal_results_dir,
)
...
def evaluate_config_on_validation(config: Dict[str, object], validation_bundles: List[Dict[str, object]]) -> Dict[str, object]:
    ...
    for bundle in validation_bundles:
        ...
        simulation = simulate_next_day_strategy(bundle["segment_dataset"]["base"], pred, initial_position=current_position)
...
def worker_process_window(task: Tuple[str, Dict[str, str]]) -> Dict[str, object]:
    ...
    frame = load_research_experiment_frame(stem)
    ...
    validation_bundles = [
        {
            **build_segment_supervised_bundle(frame, features, segment["start"], segment["end_exclusive"], penalty=JUMP_PENALTY),
    ...
    for config in CONFIG_GRID:
        ...
        selected_payloads[row["config_id"]] = {"validation": validation_result, "oos": oos_result}
...
def plot_predicted_vs_buyhold_with_bear_shading(daily_equity: pd.DataFrame, labels: pd.DataFrame, output_path: str, title: str) -> None:
...
def run_asset_pipeline(ticker: str, stem: str) -> Dict[str, object]:
    ...
    first_execution_date = pd.to_datetime(window_results[0]["oos"]["oos_dataset"]["execution_date"][0])
    buyhold_frame = build_buy_and_hold(frame, first_execution_date)
    ...
    output_dir = signal_asset_results_dir(stem)
    ...
    plot_predicted_vs_buyhold_with_bear_shading(
        daily_equity,
        labels,
        str(output_dir / f"predicted_vs_buyhold_with_bear_shading_{stem}.png"),
    ...
    return {
        ...
        "first_execution_date": first_execution_date.strftime("%Y-%m-%d"),
    }
...
def build_multi_asset_panels(asset_results: List[Dict[str, object]]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for result in asset_results:
        ...
        frame["asset_research"] = result["stem"]
        long_rows.append(
            frame[
                [
                    "Date",
    ...
    long_panel = pd.concat(long_rows, ignore_index=True).sort_values(["Date", "asset_research"]).reset_index(drop=True)
    wide = None
    for result in asset_results:
        stem = result["stem"]
    ...
    return long_panel, wide.sort_values("Date").reset_index(drop=True)
...
def main() -> None:
    ...
    signal_results_dir().mkdir(parents=True, exist_ok=True)
    performance_summary.to_csv(signal_results_dir() / "asset_performance_summary.csv", index=False)
    long_panel.to_csv(signal_panels_dir() / "multi_asset_signal_panel.csv", index=False)
    wide_panel.to_csv(signal_panels_dir() / "multi_asset_signal_panel_wide.csv", index=False)

    elapsed_seconds = time.perf_counter() - start_time
    print(f"Assets processed: {[stem for _, stem in RESEARCH_ASSETS]}")
    print(f"Target OOS start: {TARGET_OOS_START.strftime('%Y-%m-%d')}")
    print(f"Elapsed seconds: {elapsed_seconds:.2f}")
    print(f"Signal results directory: {signal_results_dir()}")
