...
RESULTS_SUBDIR = "single_threshold_dd_extra_entry_grid_gspc_to_spy"
PROB_FLOORS = [0.46, 0.48, 0.50, 0.52]
...
def floor_version(prob_floor: float) -> str:
    return f"single_threshold_055_dd20_pf{int(round(prob_floor * 100)):02d}"
...
def main() -> None:
    ...
    configs = [
        StageConfig(
            stage_name="single_threshold_055_baseline",
    ...
    configs.extend(
        [
            StageConfig(
                stage_name=floor_version(prob_floor),
                results_subdir=RESULTS_SUBDIR,
                feature_mode="enhanced",
                rule_mode="single_threshold_drawdown_overlay",
                threshold=0.55,
                drawdown_threshold=0.20,
                drawdown_prob_floor=prob_floor,
            )
    ...
    ax.set_title("Single Threshold 0.55 with Drawdown Extra Entry Grid")
    ax.set_xlabel("Date")
    ax.set_ylabel("Equity")
    ax.grid(axis="y", alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(out_dir / "single_threshold_dd_extra_entry_equity_curves.png", dpi=150)
    plt.close(fig)
    ...
    ax.grid(axis="y", alpha=0.3)
    fig.tight_layout()
    fig.savefig(out_dir / "single_threshold_dd_extra_entry_sharpe.png", dpi=150)
    plt.close(fig)
    ...
    ax.grid(axis="y", alpha=0.3)
    fig.tight_layout()
    fig.savefig(out_dir / "single_threshold_dd_extra_entry_drawdown.png", dpi=150)
    plt.close(fig)
