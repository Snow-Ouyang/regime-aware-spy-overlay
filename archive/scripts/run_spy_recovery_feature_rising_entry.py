...
RESULTS_DIR_NAME = "spy_recovery_feature_rising_entry"
BASELINE_VERSION = "current_baseline"
RECOVERY_VERSION = "recovery_features_rising_entry_rule"
ADDED_RECOVERY_FEATURES = [
    "ma20_over_ma60",
    "close_over_ma120",
    "ret_61_120d",
]
RISING_ENTRY_FLOOR = 0.60
...
def score_validation_bundle_rising_entry(
...
def select_best_smoothing_rising_entry(validation_bundles: List[Dict[str, object]]) -> Tuple[int, pd.DataFrame]:
    candidate_rows: List[Dict[str, object]] = []
    for smoothing_halflife in base.SMOOTHING_GRID:
        result = score_validation_bundle_rising_entry(
            validation_bundles,
            smoothing_halflife,
...
def select_best_double_threshold_rising_entry(
    validation_bundles: List[Dict[str, object]],
    smoothing_halflife: int,
    ...
    for lower_threshold in base.LOWER_THRESHOLD_GRID:
        for upper_threshold in base.UPPER_THRESHOLD_GRID:
            if upper_threshold <= lower_threshold:
                continue
            result = score_validation_bundle_rising_entry(validation_bundles, smoothing_halflife, lower_threshold, upper_threshold)
            candidate_rows.append(
                {
...
def worker_process_window_recovery(window_payload: Dict[str, str]) -> Dict[str, object]:
    ...
    selected_smoothing, _ = select_best_smoothing_rising_entry(validation_bundles)
    selected_double, _ = select_best_double_threshold_rising_entry(validation_bundles, selected_smoothing)
    ...
    validation_result = score_validation_bundle_rising_entry(
        validation_bundles,
...
def plot_equity_curves(daily_equity: pd.DataFrame, output_path: Path) -> None:
    ...
    ax.plot(daily_equity["Date"], daily_equity["equity_recovery_features_rising_entry_rule"], label=RECOVERY_VERSION, linewidth=1.8)
    ax.plot(daily_equity["Date"], daily_equity["equity_buy_and_hold"], label="buy_and_hold", linewidth=1.8)
    ax.set_title("SPY Recovery Features + Rising Entry Rule")
    ax.set_xlabel("Date")
...
def main() -> None:
    ...
    daily_equity = baseline_daily.merge(
        recovery_frame[["Date", "equity_predicted_strategy"]].rename(
            columns={"equity_predicted_strategy": "equity_recovery_features_rising_entry_rule"}
        ),
        on="Date",
        how="inner",
    )
    ...
    plot_equity_curves(daily_equity, out_dir / "recovery_feature_rising_entry_equity_curves.png")
    plot_metric_bar(strategy_summary, "sharpe", "Sharpe Comparison", out_dir / "recovery_feature_rising_entry_sharpe_comparison.png")
    plot_metric_bar(strategy_summary, "max_drawdown", "Max Drawdown Comparison", out_dir / "recovery_feature_rising_entry_drawdown_comparison.png")
