...
RESULTS_DIR_NAME = "spy_recovery_feature_rising_entry_grid_refined"
...
RISING_ENTRY_FLOOR_GRID = [0.48, 0.49, 0.50, 0.51, 0.52, 0.53]
FIXED_VERSION_NAMES = {
    0.48: "fixed_rising_floor_048",
    0.49: "fixed_rising_floor_049",
    0.50: "fixed_rising_floor_050",
    0.51: "fixed_rising_floor_051",
    0.52: "fixed_rising_floor_052",
...
def plot_equity_curves(daily_equity: pd.DataFrame, output_path: Path) -> None:
    ...
    for version in plot_versions:
        if version == BUYHOLD_VERSION:
            ...
            ax.plot(daily_equity["Date"], daily_equity["equity_" + version], label=version, linewidth=1.3)
    ax.set_title("SPY Recovery Features Rising-Entry Grid Refined")
    ax.set_xlabel("Date")
...
def main() -> None:
    ...
    baseline_daily = baseline_daily.rename(columns={"equity_predicted_strategy": "equity_" + BASELINE_VERSION})
    ...
    for version, final_frame in final_frames.items():
        daily_equity = daily_equity.merge(
            final_frame[["Date", "equity_predicted_strategy"]].rename(columns={"equity_predicted_strategy": "equity_" + version}),
            on="Date",
            how="inner",
        )
