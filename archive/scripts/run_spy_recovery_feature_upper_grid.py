...
RESULTS_DIR_NAME = "spy_recovery_feature_upper_grid"
BASELINE_VERSION = "current_baseline"
RECOVERY_VERSION = "baseline_plus_recovery_features_new_threshold"
ADDED_RECOVERY_FEATURES = [
    "ma20_over_ma60",
    "close_over_ma120",
    "ret_61_120d",
]
UPPER_THRESHOLD_GRID = [0.55, 0.58, 0.60, 0.63, 0.65]
...
def select_best_double_threshold_recovery(validation_bundles: List[Dict[str, object]], smoothing_halflife: int) -> Tuple[Dict[str, object], pd.DataFrame]:
    candidate_rows: List[Dict[str, object]] = []
    for lower_threshold in base.LOWER_THRESHOLD_GRID:
        for upper_threshold in UPPER_THRESHOLD_GRID:
            if upper_threshold <= lower_threshold:
                continue
            result = base.score_validation_bundle(validation_bundles, smoothing_halflife, lower_threshold, upper_threshold)
...
def worker_process_window_recovery(window_payload: Dict[str, str]) -> Dict[str, object]:
    ...
    selected_double, _ = select_best_double_threshold_recovery(validation_bundles, selected_smoothing)
...
def plot_equity_curves(daily_equity: pd.DataFrame, output_path: Path) -> None:
    ...
    ax.plot(
        daily_equity["Date"],
        daily_equity["equity_baseline_plus_recovery_features_new_threshold"],
        label=RECOVERY_VERSION,
        linewidth=1.8,
    )
    ax.plot(daily_equity["Date"], daily_equity["equity_buy_and_hold"], label="buy_and_hold", linewidth=1.8)
    ax.set_title("SPY Recovery Feature Test With Lower Upper-Threshold Grid")
    ax.set_xlabel("Date")
...
def main() -> None:
    ...
    daily_equity = baseline_daily.merge(
        recovery_frame[["Date", "equity_predicted_strategy"]].rename(
            columns={"equity_predicted_strategy": "equity_baseline_plus_recovery_features_new_threshold"}
        ),
        on="Date",
        how="inner",
    )
    ...
    feature_columns_df = pd.DataFrame(
        ...
        + [{"group": "upper_threshold_grid", "feature": str(value)} for value in UPPER_THRESHOLD_GRID]
